...
class RNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            Wx = tf.get_variable("W_x", shape=(self.input_size, self.state_size), initializer=init)
            Wh = tf.get_variable("W_h", shape=(self.state_size, self.state_size), initializer=init)
            b = tf.get_variable("b", shape=(self.state_size,), initializer=tf.zeros_initializer())

            z = tf.add(tf.matmul(inputs, Wx), tf.matmul(state, Wh))
            z = tf.add(z, b)
